...
print("=== PHASE 4: ULTRA-FAST TRAINING & THRESHOLD CALIBRATION ===", flush=True)
...
def fast_pair_features(a1, a2, tid, is_match):
    ...
    # Compute sequence ratio only for candidate pairs that passed preliminary filter
    n_char = SequenceMatcher(None, a1['norm_n'], a2['norm_n']).ratio() if n_overlap > 0 or pref3 == 1.0 else 0.0
    a_char = SequenceMatcher(None, a1['norm_a'], a2['norm_a']).ratio() if a_overlap > 0 else 0.0
...
for s1_id, a1 in s1_attr.items():
    ...
    for tid in full_cands:
        ...
        fvec = fast_pair_features(a1, a2, tid, is_match)
        if fvec is None:
            continue
