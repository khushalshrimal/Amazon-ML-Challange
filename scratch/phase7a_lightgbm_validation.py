...
print("=== PHASE 7A: LIGHTGBM MODEL VALIDATION & THRESHOLD CALIBRATION ===", flush=True)
...
# 4. STAGE 3 — LIGHTGBM MODEL TRAINING
print("\n--- STAGE 3: LightGBM Model Training ---", flush=True)
t_model = time.time()
...
print(f"Model Training (LightGBM LGBMClassifier) complete in {runtimes['Model Training']:.2f}s.", flush=True)
...
print(f"Saved LightGBM model artifact to {model_path}", flush=True)

# 5. STAGE 4 — THRESHOLD EVALUATION
...
if val_pairs_list:
    val_probs = model.predict_proba(np.array(val_pairs_list))[:, 1]
else:
    val_probs = np.array([])
...
for T in thresholds:
    ...
    if macro_f05 > best_f05:
        ...
        best_prec = prec
        best_rec = rec
        best_pred_matches = total_predicted_matches
        best_zero_s1 = zero_match_s1_count

runtimes['Threshold Evaluation'] = time.time() - t_thresh
total_pipeline_runtime = time.time() - t0_total

print(f"\nPHASE 7A OPTIMAL THRESHOLD: T* = {best_thresh:.2f} | Macro F0.5 = {best_f05:.4f} (Precision = {best_prec:.4f}, Recall = {best_rec:.4f})", flush=True)
print(f"Total Pipeline Runtime: {total_pipeline_runtime:.2f}s", flush=True)

# 6. SAVE DOCS/PHASE7_LIGHTGBM_VALIDATION.MD
doc_path = base_dir / "docs" / "PHASE7_LIGHTGBM_VALIDATION.md"
with open(doc_path, "w", encoding="utf-8") as f:
    f.write(f"""# Phase 7A — LightGBM Model Validation & Threshold Calibration

## 1. Executive Summary & Compliance
* **Model Class:** `lightgbm.LGBMClassifier` (Version {lgb.__version__})
...
| Model Training (LightGBM fit) | {runtimes['Model Training']:.2f} s |
...
""")
    for r in results:
        ...
        f.write(f"| {r['Threshold']:.2f} | {marker}{r['Macro F0.5']:.4f}{marker} | {r['Precision']:.4f} | {r['Recall']:.4f} | {r['Predicted Matches']:,} | {r['Zero-Match S1 Count']:,} |\n")

    f.write(f"""
---

## 4. Key Performance Highlights & Comparison
* **Optimal Decision Threshold:** **$T^* = {best_thresh:.2f}$**
* **Macro $F_{{0.5}}$ Score:** **{best_f05:.4f}** (Precision = {best_prec:.4f}, Recall = {best_rec:.4f})
...
print(f"\nSaved Phase 7A report to {doc_path}", flush=True)
...
if os.path.exists(log_csv):
    ...
    new_entry = {
        'exp_id': 'EXP_009',
        'phase': 'Phase 7A',
        'description': 'LightGBM LGBMClassifier model training & threshold calibration (MIT License compliance)',
    ...
    print(f"Updated experiment log at {log_csv} with EXP_009.", flush=True)

print("=== PHASE 7A VALIDATION COMPLETE ===", flush=True)
