...
print("=== PHASE 4: 1,000-ENTITY SANITY TEST PIPELINE ===", flush=True)

runtimes = {}

# 1. LOAD 1,000 GT S1 ENTITIES
t0 = time.time()
...
runtimes['blocking'] = time.time() - t_block
print(f"Candidate Generation complete in {runtimes['blocking']:.2f}s.", flush=True)

# Compute Blocking Recall for 1,000 S1 entities
...
print(f"1,000 S1 Entities Candidate Count: {total_cands_generated:,} pairs (Avg: {total_cands_generated/1000:.2f}/S1).", flush=True)
...
train_s1_ids = set(all_s1_ids[:700])
val_s1_ids = set(all_s1_ids[700:])
...
# 4. CHECK MODEL AVAILABILITY (LIGHTGBM & XGBOOST)
print("\n--- STAGE 3: Model Availability Check ---", flush=True)
has_lgb = False
has_xgb = False
...
has_allowed_model = has_lgb or has_xgb
model_status = "Available (LightGBM/XGBoost)" if has_allowed_model else "NOT INSTALLED (LightGBM and XGBoost missing)"
...
if has_lgb:
    ...
    model = lgb.LGBMClassifier(random_state=42, n_estimators=50, max_depth=5, verbose=-1)
    model_name = "LightGBM (MIT License)"
elif has_xgb:
    import xgboost as xgb
    model = xgb.XGBClassifier(random_state=42, n_estimators=50, max_depth=5)
    model_name = "XGBoost (Apache 2.0 License)"
else:
    model = HistGradientBoostingClassifier(random_state=42, max_iter=50, min_samples_leaf=15)
    model_name = "HistGradientBoostingClassifier (sklearn / BSD-3-Clause fallback)"

model.fit(X_train, y_train)
runtimes['model_training'] = time.time() - t_model
print(f"Model Training ({model_name}) complete in {runtimes['model_training']:.2f}s.", flush=True)

# 5. STAGE 4 — THRESHOLD EVALUATION (0.50, 0.60, 0.70, 0.80, 0.90)
...
for T in thresholds:
    ...
    singletons = res.get('exact_singletons_credit', 1.0)
...
print(f"\nSANITY TEST OPTIMAL THRESHOLD: T* = {best_thresh:.2f} | Macro F0.5 = {best_f05:.4f} (Precision = {best_prec:.4f}, Recall = {best_rec:.4f})", flush=True)

# SAVE DOCS/PHASE4_SANITY_TEST.MD
doc_path = base_dir / "docs" / "PHASE4_SANITY_TEST.md"
with open(doc_path, "w", encoding="utf-8") as f:
    f.write(f"""# Phase 4 — 1,000-Entity Sanity Test Results
...
* **Ground-Truth True Pairs:** {len(val_gt_pairs):,} true match pairs in 1,000 S1 sample.
* **Candidate Pairs Generated:** {total_cands_generated:,} candidates (Avg: {total_cands_generated/1000:.2f} candidates/S1).
...
* **Model Used for Sanity Evaluation:** `{model_name}`
...
|---|---|
| Candidate Generation (blocking) | {runtimes['blocking']:.2f} s |
...
| **Total Sanity Test Runtime** | **{sum(runtimes.values()):.2f} s** |
...
* **Pipeline Speed:** The 1,000-entity sanity test completed cleanly in **{sum(runtimes.values()):.2f} seconds** without any runtime bottleneck.
* **Optimal Sanity Threshold:** **$T^* = {best_thresh:.2f}$** achieved a **Macro $F_{{0.5}}$ score of {best_f05:.4f}** (Precision = {best_prec:.4f}, Recall = {best_rec:.4f}).
...
print(f"\nSaved Sanity Test report to {doc_path}", flush=True)
