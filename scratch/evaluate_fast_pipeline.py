"""
Fast Baseline Validation Evaluator for Submission 2 (Section 3).

Uses AdvancedBlocker with posting limit = 100 to eliminate candidate explosions,
enabling fast feature extraction and evaluation on 10k validation set.
Logs live telemetry every batch.
"""
...
def main():
    print("=" * 70, flush=True)
    print("FAST BASELINE VALIDATION EVALUATION (10K VALIDATION SET)", flush=True)
    print("=" * 70, flush=True)
    ...
    print("\n[STAGE 1/3] Building Advanced Blocker & Target Store...", flush=True)
    ...
    print("\n[STAGE 2/3] Loading LightGBM baseline model...", flush=True)
    lgb_model = joblib.load(MODEL_PATH)

    print("\n[STAGE 3/3] Feature extraction & prediction loop...", flush=True)
    ...
    for idx, s1_rec in enumerate(s1_recs):
        ...
        if (idx + 1) % 2000 == 0 or (idx + 1) == total_s1 or time.time() - last_print > 5.0:
            ...
            print(f"[EVALUATING FAST BASELINE] S1: {idx+1}/{total_s1} ({((idx+1)/total_s1)*100:.1f}%) | "
    ...
    print("\n" + "=" * 70, flush=True)
    print("BASELINE VALIDATION METRICS (10K VALIDATION SPLIT)", flush=True)
    print("=" * 70, flush=True)
    print(f"Macro F0.5 Score   : {metrics['macro_f05']:.4f}", flush=True)
    print(f"Macro Precision    : {metrics['macro_precision']:.4f}", flush=True)
    print(f"Macro Recall       : {metrics['macro_recall']:.4f}", flush=True)
    print(f"Candidate Recall   : {(gt_pairs_found / total_gt_matches)*100:.2f}% ({gt_pairs_found}/{total_gt_matches})", flush=True)
    print(f"Candidate Count    : {candidate_pairs_count:,} ({candidate_pairs_count/total_s1:.1f} cands/S1)", flush=True)
    print(f"Matched S1 Count   : {pred_matches_cnt:,} (Unmatched: {unmatched_cnt:,})", flush=True)
