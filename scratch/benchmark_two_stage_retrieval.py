...
class CheapBlocker:
    ...
    def get_cheap_ranked_candidates(self, s1_prep: dict, target_store: CheapTargetStore, top_k: int = None) -> list:
        c = s1_prep['c']
        nn = s1_prep['norm_n']
        na = s1_prep['norm_a']
...
def run_benchmark():
    print("=" * 80)
    print("TWO-STAGE RETRIEVAL & ML SCORING BENCHMARK (K = None, 200, 100, 50, 20)")
    print("=" * 80)
    ...
    k_values = [None, 200, 100, 50, 20]
    benchmark_results = {}

    for K in k_values:
        label = "No Cap (All)" if K is None else f"Top K={K}"
        print(f"\n" + "-" * 60)
    ...
    print("=" * 80)
    print(f"{'K Cap':<15} | {'Cand Rec':<10} | {'Avg Cand':<10} | {'F0.5':<8} | {'Prec':<8} | {'Rec':<8} | {'10k Time':<10} | {'Est Full Test':<14}")
    print("-" * 95)
    for label, r in benchmark_results.items():
        print(f"{str(r['K']):<15} | {r['candidate_recall']:<10} | {r['avg_candidates_per_s1']:<10} | {r['macro_f05']:<8} | {r['macro_precision']:<8} | {r['macro_recall']:<8} | {r['runtime_10k_s1_sec']:<10} | {r['est_full_test_hours']:<14}")
